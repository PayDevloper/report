...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : 'NavigationController.do?navidatagrid',
			toolbar : '#toolbar',
			remoteSort : false,
			title : '',
			iconCls : 'icon-save',
				...
				handler : function() {
					if (userForm.find('[name=id]').val() != '') {
						userForm.form('submit', {
							url : 'NavigationController.do?editsubnavi',
						...
						userForm.form('submit', {
							url : 'NavigationController.do?addsubnavi',
							success : function(data) {
								try {
	...
	function seltotalitem() {
		window.open('/platform/NavigationController.do?navisel');
	}
	function edit() {
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项？', function(r) {
				if (r) {
					...
					$.ajax({
						url : 'NavigationController.do?delsubnavi',
						data : {
							ids : ids.join(',')
						},
						cache : false,
	...
	<div id="userDialog" style="display: none; overflow: hidden;">
		<form id="userForm" method="post">
			...
			<table class="tableForm">
				...
				<tr>
					<th>所属一级导航</th>
					<td><input name="totalname" class="easyui-validatebox" id="totalname"
						required="true" />
						<input type="button" name="seltotal" onclick="seltotalitem()"
						style="width: 60px;" id="seltotal" value="选择"> 
						<input name="subname" id="subname"
