...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : 'AuthmanageController.do?datagrid',
			toolbar : '#toolbar',
			remoteSort : false,
			title : '',
			iconCls : 'icon-save',
				...
				handler : function() {
					if (userForm.find('[name=id]').val() != '') {
						userForm.form('submit', {
							url : 'AuthmanageController.do?edit',
						...
						userForm.form('submit', {
							url : 'AuthmanageController.do?add',
							success : function(data) {
								try {
	...
	function seltotalitem() {
		window.open('/platform/AuthmanageController.do?usersel');
	}
	function edit() {
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项？', function(r) {
				if (r) {
					...
					$.ajax({
						url : 'AuthmanageController.do?del',
						data : {
							ids : ids.join(',')
						},
						cache : false,
	...
	<div id="userDialog" style="display: none; overflow: hidden;">
		<form id="userForm" method="post">
			...
			<table class="tableForm">
				<tr>
					<th>岗位名称</th>
					<td><input name="subname" class="easyui-validatebox"
						required="true" /></td>
				</tr>
				<tr>
					<th>用户名称</th>
					<td><input name="totalname" id="totalname"
						class="easyui-validatebox" readonly="readonly" required="true" />
						<input type="button" name="seltotal" onclick="seltotalitem()"
						style="width: 60px;" id="seltotal" value="选择"></td>
				</tr>
				<tr>
					<th>用户编号</th>
